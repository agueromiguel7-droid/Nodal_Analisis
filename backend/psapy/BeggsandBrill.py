# coding=utf-8
import backend.psapy.FluidProps as FluidProps
import math

def Pgrad(P, T, oil_rate, wtr_rate, Gor, gas_grav, oil_grav, wtr_grav, d, angle, pvt_model="standing"):
    ...
    Pb = FluidProps.Pbub(T, Tsep, Psep, gas_grav, oil_grav, Gor, pvt_model)                   #Bubble point pressure, psia
    Rso = FluidProps.sol_gor(T, P, Tsep, Psep, Pb, gas_grav, oil_grav, pvt_model)             #Solution gas-oil ratio, scf/stb
    Rsw = FluidProps.sol_gwr(P, T, TDS)                                            #Solution gas_water ratio, scf/stb
    Bo = FluidProps.oil_fvf(T, P, Tsep, Psep, Pb, Rso, gas_grav, oil_grav, pvt_model)         #Oil formation volume factor, rb/stb
    ...
    ratio_safe = abs(rhol / sigl) if sigl != 0 else 1.0
    Nvl = 1.938 * usl * (ratio_safe) ** 0.25                            #Liquid velocity number
    
    laml = usl / um if um > 0 else 1.0                                      #Input liquid fraction
    # Mathematical Cap to prevent L1, L2, L3, L4 exponential explosion at boundaries
    laml_safe = max(min(laml, 0.95), 0.05) 
    
    lamg = 1 - laml                                                     #Input gas fraction
    L1 = 316 * laml_safe ** 0.302                                             #Dimensionless constants
    L2 = 0.0009252 * laml_safe ** -2.4684
    L3 = 0.1 * laml_safe ** -1.4516
    L4 = 0.5 * laml_safe ** -6.738
...
def Liq_holdup(Nfr, Nvl, laml, angle, regime):
    ...
    #Calculate holdup
    Nvl_safe = max(Nvl, 1e-6)
    Nfr_safe = max(Nfr, 1e-6)
    laml_safe = max(laml, 1e-6)
    try:
        log_term = math.log(d) + e * math.log(laml_safe) + f * math.log(Nvl_safe) + g * math.log(Nfr_safe)
    except:
        log_term = 0
    corr = (1 - laml) * log_term
    if (corr < 0):
        corr = 0
...
def Fric(Nre, eps):
    ...
    try:
        inner_log = (eps ** 1.1098 / 2.8257) + (7.149 / max(Nre, 1e-6)) ** 0.8981
        term = (eps / 3.7065) - (5.0452 / max(Nre, 1e-6)) * math.log10(max(inner_log, 1e-10))
        Temp = -4 * math.log10(max(term, 1e-10)) 
    except Exception as inst:
         Temp = -10.0 # fallback for turbulent fanning friction approx 0.01
...
def  Pgrad2(P, T, oil_rate, wtr_rate, Gor, gas_grav, oil_grav, wtr_grav, d, angle):
    ...
    #Calculate fluid properties
    Z = FluidProps.zfact(Tr = (T + 460) / FluidProps.Tc(gas_grav), Pr = P / FluidProps.Pc(gas_grav))               #Gas compressibility factor
    Wor = wtr_rate / oil_rate                                           #Water-oil ratio, stb/stb
    TDS = FluidProps.salinity(wtr_grav)                                            #Water salinity, wt% total dissolved solids
...
def Pwf_q(FWHP, FWHT,Oil_Rate,Water_Rate,GOR,GasGrav,API, WaterGrav, ID, Angle, Depth, FBHT, pvt_model="standing"):
    ...
    while (i<= nSteps):
        ...
        dp= Pgrad(p,T,Oil_Rate,Water_Rate,GOR,GasGrav,API, WaterGrav, ID, Angle, pvt_model)
        DPs.append(dp)
