...
def Pbub(T, Tsep, Psep, gas_grav, oil_grav, Gor, model="standing"):
    """Function to Calculate Bubble Point Pressure in psia"""
    gas_grav_corr = correct(Tsep, Psep, gas_grav, oil_grav)
    if model == "vasquez_beggs":
        if (oil_grav <= 30):
            C1, C2, C3 = 0.0362, 1.0937, 25.724
        else:
            C1, C2, C3 = 0.0178, 1.187, 23.931
        return (Gor / (C1 * gas_grav_corr * math.exp(C3 * oil_grav / (T + 460)))) ** (1 / C2)
    elif model == "glaso":
        Pb_star = (Gor / gas_grav)**0.816 * (T + 460)**0.172 / max(oil_grav, 1e-5)**0.989
        if Pb_star <= 0: return 14.7
        log_Pb_star = math.log10(Pb_star)
        log_Pb = 1.7669 + 1.7447 * log_Pb_star - 0.30218 * log_Pb_star**2
        return 10**log_Pb
    else: # standing
        return 18.2 * ((Gor / gas_grav)**0.83 * 10**(0.00091 * T - 0.0125 * oil_grav) - 1.4)
...
def sol_gor(T, P, Tsep, Psep, Pb, gas_grav, oil_grav, model="standing"):
    ...
    if model == "vasquez_beggs":
        if (oil_grav <= 30):
            ...
            C1, C2, C3 = 0.0178, 1.187, 23.931
        return C1 * gas_grav_corr * P_eff ** C2 * math.exp(C3 * oil_grav / (T + 460))
    elif model == "glaso":
        log_P = math.log10(max(P_eff, 1e-5))
        a, b, c = 0.30218, -1.7447, log_P - 1.7669
        disc = max(b**2 - 4*a*c, 0)
        x = (-b - math.sqrt(disc)) / (2*a)
        Pb_star = 10**x
        return gas_grav * (Pb_star * oil_grav**0.989 / (T + 460)**0.172)**(1/0.816)
    else: # standing
        return gas_grav * (((P_eff / 18.2) + 1.4) * 10**(0.0125 * oil_grav - 0.00091 * T))**(1/0.83)

def oil_fvf(T, P, Tsep, Psep, Pb, Rs, gas_grav, oil_grav, model="standing"):
    ...
    if model == "vasquez_beggs":
        ...
        gamma_o = 141.5 / (oil_grav + 131.5)
        Bob_star = Rs * (gas_grav / gamma_o)**0.526 + 0.968 * T
        log_Bob_star = math.log10(max(Bob_star, 1e-5))
        Bob = 1.0 + 10**(-1.223 + 2.258 * log_Bob_star - 0.744 * log_Bob_star**2)
    else: # standing
        gamma_o = 141.5 / (oil_grav + 131.5)
        F = Rs * (gas_grav / gamma_o)**0.5 + 1.25 * T
...
def zfact(Tr, Pr):
    ...
    #'Pr         reduced pressure
    a = 1.39 * abs(Tr - 0.92) ** 0.5 - 0.36 * Tr - 0.101
...
def  gvisc(P, T, Z, grav):
    ...
    if rho < 0: rho = 1e-6 # Physical safeguard: density cannot be negative
